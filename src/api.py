...
from models import get_session, add_crypto_info
import os
...
#                             success = await push_to_channel(FakeContext(), message, crypto_id, session)

#                             if success:
#                                 logger.info(f"成功推送代幣通知: {token_address}")
#                             else:
#                                 logger.error(f"推送代幣通知失敗: {token_address}")
                ...
                if should_process:
                    ...
                    try:
                        ...
                        try:
                            ...
                            # 向所有語言主題推送
                            results = await push_to_all_language_channels(FakeContext(), crypto_data, session)
                            ...
                            if "error" in results:
                                ...
                                if success_count == total_count:
                                    logger.info(f"成功推送代幣通知到所有 {total_count} 個語言群組: {token_address}")
                                else:
...
async def fetch_token_info(token_address: str) -> Optional[Dict]:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async with session.get(url, headers=headers) as response:
            ...
            if token_data.get("price") is None or token_data.get("market_cap") is None or token_data.get("symbol") is None:
                return None
...
# 修改批量添加代幣API
@app.route('/api/tg_push_batch', methods=['POST'])
async def tg_push_batch():
    """批量接收多個代幣地址並異步觸發推送"""
    try:
        # 獲取請求數據
        data = await request.get_json()
        ...
        if not token_addresses or not isinstance(token_addresses, list):
            return jsonify({
                'status': 'error',
                'message': 'Missing or invalid token_addresses parameter: must be a list'
            }), 400
        ...
        # 過濾已處理的代幣
        async with processing_lock:
            filtered_tokens = [addr for addr in token_addresses if addr not in processed_tokens]

        # 將任務添加到隊列
        count = 0
        with queue_lock:
            for token_address in filtered_tokens:
                logger.info(f"將代幣添加到處理隊列: chain={chain}, address={token_address}")
                token_queue.append({
                    'token_address': token_address,
                    'chain': chain
        ...
        return jsonify({
            'status': 'success',
            'message': f'{count} token notification requests accepted and queued for processing',
            'queued': count,
            'skipped': len(token_addresses) - count
        })

    except Exception as e:
        logger.error(f"批量API請求處理錯誤: {e}")
        return jsonify({
            'status': 'error',
            'message': str(e)
        }), 500
